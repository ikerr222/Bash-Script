...
jugadas_totales=0
bet_to_renew=$(($money+50)) #Dinero que haŕa que renovemos nuestra secuencia a [1 2 3 4]
echo -e "[+] El tope para renovar la secuencia está establecido por encima de $bet_to_renew€"
tput civis
while true; do
  ...
  if [ ! "$money" -lt 0 ]; then
      ...
      if [ $money -gt $bet_to_renew ]; then
        echo -e "${blue}[+] Se ha superado el tope establecido de $bet_to_renew€ para renovar la secuencia${end}"
        bet_to_renew=$(($bet_to_renew+50))
        echo -e "${green}[+] El tope se ha establecido en $bet_to_renew€${end}"
        my_sequence=( 1 2 3 4 )
        bet=$((${my_sequence[0]} + ${my_sequence[-1]}))
        echo -e "${yellow}[+] La secuencia ha sido restablecida a: [${my_sequence[@]}]${end}"
      else
            my_sequence+=($bet)
        my_sequence=(${my_sequence[@]})
    ...
    echo  -e "${red}[+] El número es 0, ¡Pierdes!${end}"
       ...
       echo -e "${yellow}[+] Hemos llegado a un mínimo crítico, se procede a reajustar el tope${end}"
       bet_to_renew=$(($bet_to_renew - 50))
